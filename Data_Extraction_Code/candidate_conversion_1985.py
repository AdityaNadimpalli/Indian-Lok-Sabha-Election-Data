...
valid_general=None
cat=None
f=open('../1985/1985_detailed_results_raw_output.csv')
csv_f=csv.reader(f)
flag=0
tmp1=[]
tmp2=[]
...
for row in csv_f:
	...
	if not row:
		continue
	elif("rptDetailedResults" in row[0]): #bottom of page, ignore page footer
		continue
		...
		if(row[1]!=""):
			...
			valid_total=int(tmp3[n-2])
			if("#Num!" not in tmp3[n-1]):
				percent=float(tmp3[n-1])
			else: percent=None
			sum=sum+valid_total
		else:
			print "error"
